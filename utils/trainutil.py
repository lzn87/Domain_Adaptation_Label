...
# evaluate model based on distance instead of total cost
def test_highdim_dist(model, testloader, device):
    model.eval()
    mels = torch.flatten(torch.tensor(testloader.dataset.mels, device=device), start_dim=1)

    mels_normalized = F.normalize(mels)
    acc_l1 = 0.
    acc_l2 = 0.
    acc_cos = 0.
    count = 0
    with torch.no_grad():
        for _, (inputs, targets) in enumerate(testloader):
            ...
            # L1 distance
            pred_l1 = torch.cdist(outputs, mels, p=1.).argmin(dim=1)
            # L2 distance
            pred_l2 = torch.cdist(outputs, mels).argmin(dim=1)
            # cosine distance
            pred_cos = (F.normalize(outputs) @ mels_normalized.T).argmax(dim=1)

            acc_l1 += (pred_l1 == targets).float().sum()
            acc_l2 += (pred_l2 == targets).float().sum()
            acc_cos += (pred_cos == targets).float().sum()
            count += inputs.size(0)

    acc_l1 /= count
    acc_l2 /= count
    acc_cos /= count
    print(f'L1 distance acc = {acc_l1:.3f} L2 distance acc = {acc_l2:.3f}, Cosine distance acc = {acc_cos:.3f}')
    return 0., max(acc_l1, acc_l2, acc_cos)
